...
const SignInPage: FC = () => {
	...
	return (
		<main className="min-h-screen w-screen flex items-center justify-center relative overflow-hidden bg-white dark:bg-black">
			...
			<div className="relative z-10 w-full max-w-md p-8 rounded-3xl backdrop-blur-md shadow-2xl bg-white border border-slate-200 text-slate-900 dark:bg-white/6 dark:border-white/10 dark:text-white">
				<div className="flex flex-col items-center mb-6">
					<img src="/logo.png" alt="Scholarsuite Logo" className="h-12 mb-4" />
					<h1 className="text-2xl font-bold">Scholarsuite</h1>
					<p className="text-sm text-slate-600 dark:text-white/80">Gestion de vie scolaire et de scolarité</p>
				</div>

		<h2 className="text-lg font-semibold mb-4 text-center text-slate-700 dark:text-white">Connexion</h2>

			<form onSubmit={handleSubmit} className="space-y-4">
					<div>
						<label htmlFor="email" className="block text-sm font-medium mb-2 text-slate-700 dark:text-white/80">Email</label>
					...
					<div>
						<label htmlFor="password" className="block text-sm font-medium mb-2 text-slate-700 dark:text-white/80">Mot de passe</label>
						<input
							id="password"
							type="password"
							value={password}
							...
							required
							className={
								`w-full px-4 py-3 rounded-xl shadow-sm focus:outline-none transition duration-150 bg-slate-50 border border-slate-200 text-slate-900 placeholder:text-slate-400 dark:bg-white/6 dark:border-white/10 dark:text-white dark:placeholder:text-white/50 dark:focus:ring-white/20 dark:focus:border-white`
							}
						/>
					</div>
					...
					>
						...
						<span className="z-10 font-semibold">{loading ? 'Connexion...' : "Se connecter"}</span>
					</button>
				</form>
				<div className="mt-6">
					<div className="flex items-center my-4">
						<span className="flex-1 h-px bg-slate-200 dark:bg-white/10" />
						<span className="mx-3 text-sm text-slate-500 dark:text-white/70">ou avec</span>
						<span className="flex-1 h-px bg-slate-200 dark:bg-white/10" />
					</div>
